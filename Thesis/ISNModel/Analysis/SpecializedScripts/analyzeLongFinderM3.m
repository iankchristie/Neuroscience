    ...
    for w = 1: length(weak),
        if strong(s) > weak(w),
            ...
            if ~isempty(ds) && ~isempty(uni),
                for i = 1: length(ds),
                    for j = 1: length(uni),
                        if ds{i}.Wxi1 == uni{j}.Wxi1 && ds{i}.Wxi2 == uni{j}.Wxi2,
                            if works(1,end) ~= strong(s) && works(2,end) ~= weak(w),
...
for k = 1: length(works),
    if works(1,k) > works(2,k),
        load(sprintf('LongFinderM3Data/S%dW%d/AllDS.mat', works(1,k),works(2,k)));
        load(sprintf('LongFinderM3Data/S%dW%d/AllUni.mat', works(1,k),works(2,k)));
        load(sprintf('LongFinderM3Data/S%dW%d/AllHalfUni.mat', works(1,k),works(2,k)));
