...
for  s = 1: length(strong),
    for w = 1: length(weak),
        if strong(s) > weak(w),
            load(sprintf('LongFinderM2Data/S%dW%d/AllDS.mat', strong(s),weak(w)));
            load(sprintf('LongFinderM2Data/S%dW%d/AllUni', strong(s),weak(w)));
...
for k = 1: length(works),
    if works(1,k) > works(2,k),
            load(sprintf('LongFinderM2Data/S%dW%d/AllDS.mat', works(1,k),works(2,k)));
            load(sprintf('LongFinderM2Data/S%dW%d/AllUni.mat', works(1,k),works(2,k)));
            load(sprintf('LongFinderM2Data/S%dW%d/AllHalfUni.mat', works(1,k),works(2,k)));
