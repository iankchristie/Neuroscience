...
for s = 1: length(strong),
    for w = 1: length(weak),
        if strong(s) > weak(w),
            ...
            save(sprintf('ParFinderM2Data/S%dW%d/AllDS.mat', strong(s),weak(w)), 'ds');
            save(sprintf('ParFinderM2Data/S%dW%d/AllUni.mat', strong(s),weak(w)), 'uni');
            save(sprintf('ParFinderM2Data/S%dW%d/AllHalfUni.mat', strong(s),weak(w)), 'halfuni');
            ds = {};
            uni = {};
            halfuni = {};
        end
