...
for s = 1: length(strong),
    for w = 1: length(weak),
        if strong(s) > weak(w),
            ...
            for i = 1: length(Wxe),
                for j = 1: length(Wxi1),
                    for k = 1: length(Wxi2),
                        ...
                        m = getConnectionMatrix2('Wxe1',Wxbase+Wxe(i),'Wxe2',Wxbase-Wxe(i),'Wxi1',Wxi1(j),'Wxi2',Wxi2(k));
            ...
            save(sprintf('ParFinderM3Data/S%dW%d/AllDS.mat', strong(s),weak(w)), 'ds');
            save(sprintf('ParFinderM3Data/S%dW%d/AllUni.mat', strong(s),weak(w)), 'uni');
            save(sprintf('ParFinderM3Data/S%dW%d/AllHalfUni.mat', strong(s),weak(w)), 'halfuni');
            ds = {};
            uni = {};
            halfuni = {};
        end
