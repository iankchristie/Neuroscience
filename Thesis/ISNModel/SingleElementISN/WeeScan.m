...
ExcitatorySteadyState = zeros();

for i = 1: length(Wee),
    fprintf('Wee %d \n', Wee(i));

    for q = 1: length(currents),
        column = getConnectionMatrix2('Wee', Wee(i),'Wii', Wii, 'Wei', Wei, 'Wie', Wie);
        ...
        ExcitatorySteadyState(i,q) = r(1, round(.75/dt));
    end
end
figure;
image(ExcitatorySteadyState);
