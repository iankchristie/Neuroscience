...
for i = 1: numTrials,
    temp = round(rand);
    IUpNoise = [addNoise([zeros1 strongArray zeros4 0], sigma);addNoise([zeros2 weakArray zeros3 0], sigma)];
    IDownNoise = [addNoise([zeros2 weakArray zeros3 0], sigma);addNoise([zeros1 strongArray zeros4 0], sigma)];
    if temp == 1,
        temp = 0;
    else
        temp = 1;
    ...
    if temp,
        [v1] = runModelTime(time, dT, tau, M, IUpNoise, W);
        [v2] = runModelTime(time, dT, tau, M, IDownNoise, W);
    else
        [v1] = runModelTime(time, dT, tau, M, IDownNoise, W);
        [v2] = runModelTime(time, dT, tau, M, IUpNoise, W);
    end
