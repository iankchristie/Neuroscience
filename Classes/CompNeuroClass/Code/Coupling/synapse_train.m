...
tau_d = 0.25;            % time for recovery from depression
tau_f = 0.5;            % time for recovery from facilitation
f_f = 0.2;              % increase in pr immediately following a spike
...
for n = 1:Nspikes
    ...
    for i = ispike+1:nspike-1                       % set of points to update
        ...
        p(i+1) = D(i+1)*pr(i+1);                     % update total release proportion
    end
...
figure(2)
plot(t,pr)
ylabel('Fract. of Neurotransmitter release')
xlabel('Time, sec')
