...
dt=0.005;                       % time step
tmax = 2;                       % maximum time: check rates are not changing by this time
t = 0:dt:tmax;                  % vector of time points

I_on = 0.5;
I_off = 1.0;
Ionvector = zeros(size(t));
Ionvector(round(I_on/dt):round(I_off/dt)) = 1.0;
...
J2 = 1.5;                         % Extra excitatory connection to similar cells
...
for c = 0:0.2:0.8;              % Use different contrasts stepping from 0 to 0.8
    ...
    for i = 2:length(t)         % Start simulating across time
        ...
        Itot = Ionvector(i)*h + r(i-1,:)*W;  % Total current is input current + rate of all cells * connection strength
...
xlabel('Time')
ylabel('cell number')
title('Firing rate of population')
