...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%Code Added Below%%%%%%%%%%%%%%%%%%%%%%%%%%
Amplitude1 = 100e-9;        %amplitude
VerticalShift1 = 300e-9;    %Y shift
Period = 12;                %Period
...
%create spike vector
spikes = zeros(size(t));    %initialize to 0's
threshold = 0;              %What we count as a spike
...
Amplitude = (max(VS) - min(VS))/2;
VerticalShift = (max(VS) + min(VS))/2;

adjustedIapp = Amplitude*sin(2*pi*Period*t)+VerticalShift;

figure;
plot(t,VS)
hold on
plot(t,adjustedIapp,'r');
legend('Soma Voltage', 'Applied Current');
xlabel('Time (sec)')
ylabel('Soma Voltage (v)')
title('Soma Voltage vs. time')