...
g_Ca = 0.01e-3;    % calcium conductance
g_KAHP = 8e-6;      % Potassium conductance to generate after-hyperpolarization
%g_KAHP = 0;
g_KCa = 0.5e-3;      % calcium-dependent Potassium conductance
...
convert_Ca = 20000;  % conversion changing calcium charge entry per unit area into concentration
...
%    m(i) = m_inf - (m_inf-m(i-1))*exp(-dt/tau_m);    
    ...
    alpha_mca = 1.6/( 1+exp(-0.072*(VmD+10)) );
    if ( VmD == -23.9 ) 
        beta_mca = 0.02/0.2;
    else 
        beta_mca = 0.02*(VmD+23.9)/(exp(0.2*(VmD+23.9))-1);
    end
