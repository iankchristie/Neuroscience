% HHmbramp.m Ramps up the current, then back down. 
% With no A-current, it shows hysterisis.
...
if ( iclamp_flag )   % i.e. if in current-clamp mode
    ...
    for i = 1:length(t)
        if ( t(i) < tmax/2 )
            Iapp(i) = Imin + (Imax-Imin)*t(i)*2/tmax;
        else
            Iapp(i) = Imin + (Imax-Imin)*(tmax-t(i))*2/tmax;
        end
    end
end
...
for i = 2:length(t); % now see how things change through time
    ...
    m(i) = m_inf - (m_inf-m(i-1))*exp(-dt/tau_m);    % Update m
    ...
    a(i) = a_inf - (a_inf-a(i-1))*exp(-dt/tau_a);    % Update a
    b(i) = b_inf - (b_inf-b(i-1))*exp(-dt/tau_b);    % Update b
    ...
    if ( ( inspike == 0 ) && ( V(i) > Vspike ) )
        ...
        if ( t(i) > 0.1 )
            nspikes = nspikes+1;
            current(nspikes) = Iapp(i-1);
            spiketime(nspikes) = t(i-1);
        end
    end
...
clf
ISIs = diff(spiketime);
meancurrents = current(1:length(ISIs)) + 0.5*diff(current);
meancurrents = [current(1),meancurrents ];
ISIs = [100, ISIs];

plot(meancurrents,1./ISIs,'x-')
